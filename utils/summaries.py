...
class TensorboardSummary(object):
    ...
    def visualize_image(self, writer, dataset, image, target, output, global_step, epoch, epoch_id, validation=False):
        if isinstance(output, dict):
            ...
            # deeplab_segm_map = (255*decode_seg_map_sequence(torch.max(output["psp_seg"], 1)[1].detach().cpu().numpy(), dataset=dataset).numpy()).astype(np.uint8)
            # hr_segm_map = (255*decode_seg_map_sequence(torch.max(output["ocr_seg"], 1)[1].detach().cpu().numpy(), dataset=dataset).numpy()).astype(np.uint8)
            target_map = self.decode_target(target.detach().cpu().numpy())
            pred=output['binary_segmentation']
            pred=torch.argmax(torch.softmax(pred,dim=1),dim=1)
            final_seg_map=self.decode_target(pred.detach().cpu().numpy())
            w = image.size()[3]
            h = image.size()[2]
            b = image.size()[0]
            out_img = np.zeros(shape=[4*h , b*w, 3], dtype=np.uint8)
            for i in range(0, b):
                ...
                # out_img[2*h:3*h, i*w:(i+1)*w, :] = deeplab_segm_map[i,...].transpose(1,2,0)
                # out_img[3*h:4*h, i*w:(i+1)*w, :] = hr_segm_map[i,...].transpose(1,2,0)
                out_img[2*h:3*h, i*w:(i+1)*w, :] = target_map[i, ...]
                out_img[3*h:4*h, i*w:(i+1)*w, :] = final_seg_map[i,...]                
